...
const NaiveTotalBountyFund = artifacts.require('NaiveTotalBountyFund');
const StakeToken = artifacts.require('StakeToken');
const MockedAllocator = artifacts.require('MockedAllocator');

contract.only('NaiveTotalBountyFund', (accounts) => {
  let stakeToken, bountyFund;

  beforeEach(async () => {
    stakeToken = await StakeToken.new('Lottery Oracle Token', 'LOT', 15);

    bountyFund = await NaiveTotalBountyFund.new(stakeToken.address);
  });
